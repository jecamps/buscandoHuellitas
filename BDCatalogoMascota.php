...
if ($bandera == 1) {
    ...
    $edad = $_POST["edad"]; 
    $fechaS = $_POST["fecha"]; 
    $fechaE = date("d/m/Y", strtotime($fechaS));
    ...
    if (isset($foto) && $foto != "") {
        ...
        if ( !(strpos($tipoI,'jpg') || strpos($tipoI,'png') || strpos($tipoI,'jpeg'))) {
            echo "<script text='text/javascript'>
                ...
                window.location='UnuevaMascota.php';
                </script>";
        } else {
            $insertar = "INSERT INTO `mascotas`(`id`, `nombre`, `edad`, `fecha`, `recompensa`, `descripcion`, `tipo`, `mascotaP`, `estado`, `dom`, `foto`, `nombreI`, `estatus`, `diaRegistro`, `usuario`) 
            VALUES ('','$nombre','$edad','$fechaE','$recompensa','$descripcion','$tipo','$mascotaP','$estado','$dom','$foto','fotomascota','$estatus','$hoy','$usuario')";
